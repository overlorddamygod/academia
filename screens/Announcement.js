...
const AnnouncementScreen = () => {
    ...
    const items = ["All Items","Exams","Classes","Holiday","Result","Project"]

    return (
        <View style={{flex:1}}>
            <View style={{height:StatusBar.currentHeight, backgroundColor:"#4C367B"}}></View>
            ...
            <View style={{...styles.tags,...styles.shadow}}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{height: "100%"}}>
                    {items.map((itemName,i)=> {
                        return <Item name={itemName} active={selectedTag == itemName} onPress={setSelectedTag} key={i}/>
                    })}
                </ScrollView>
            </View>
...
const Announcement = ({data= {tag:"Classes","title":"Lorem Ipsum",time:"10:00 PM"}}) => {

    return (
        <View style={{marginHorizontal: 20, marginBottom:10, backgroundColor:"white", borderRadius: 8, padding: 8,...styles.shadow}}>
            <View style={{alignSelf: 'flex-start', backgroundColor: itemTypes[data.tag], paddingHorizontal: 5, borderRadius:6}}>
                <Text style={{color: "white"}}>{data.tag}</Text>
            </View>
            <Text style={{fontWeight:"bold",fontSize:20,marginVertical:8}}>{data.title}</Text>
...
const Item = ({name, active, onPress}) => {
    return (
        <TouchableOpacity style={{alignItems:"center", textAlign:"center",marginRight: 15}} onPress={()=>{
            onPress(name)
        }}>
            <Text style={{ fontWeight: active ? "bold" : "normal" , color: !active ? "#ABABAB": "black",alignItems:"center" }}>{name}</Text>
            { active &&
